...
> Code-Metrics-Collector - Open source tool by AetherCodeHQ

`cli` `code-analysis` `developer-tools` `golang` `metrics`

## What is it?

**Code Metrics Collector** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Code-Metrics-Collector.git
cd Code-Metrics-Collector
go build -o Code-Metrics-Collector .
```

### Run
```bash
./Code-Metrics-Collector --help
```
...
```bash
# Basic usage
./Code-Metrics-Collector --path ./target

# With options
./Code-Metrics-Collector --path ./target --format json --output report.json

# Verbose mode
./Code-Metrics-Collector --path ./target --verbose
```
...
### Basic scan
```bash
./Code-Metrics-Collector --path ./my-project
```

### JSON report
```bash
./Code-Metrics-Collector --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Code Metrics Collector
  run: ./Code-Metrics-Collector --path . --format json --output report.json
```
